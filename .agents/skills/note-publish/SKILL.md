...
2.  **Refactor Content (on the copy):** Improve the text's clarity, conciseness, and impact in the new file.
3.  **Add Mermaid Diagrams (on the copy):** Identify where diagrams can clarify the text and write them using [mermaid-patterns.md](references/mermaid-patterns.md).
4.  **Update Metadata (on the copy):**
    - Set `date` to yesterday's date (matching the filename).
    - Since this text is a "Note", ensure no `layout` field is present.
...
6.  **Update About Page (Optional):** If the post represents a new certification or professional milestone, prompt the user to update `_tabs/about.md` with the relevant information (e.g., adding a new entry to the Certifications section).
7.  **Preserve Original:** Keep the original draft in `_unpublished_posts/` untouched for comparison.
8.  **Generate Flashcards (Optional):** Prompt the user to run the `anki-flashcards` skill to generate and save study material for the newly published post.
