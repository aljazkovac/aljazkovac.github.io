...
There are traditional proxies, also known as forward proxies, and reverse proxies. A forward proxy sits in front of a client and forwards its requests to the internet, while a reverse proxy sits in front of a server and handles requests coming from a client.
...
I implement another one the old-fashioned way. But the implementation felt awkward and bloated.
...
a bit to get the right types of requests, etc., but the original LLM recommendation was quite close.
