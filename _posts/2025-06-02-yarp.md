...
##### Comparing the Implementations: Why Vibe-Coding Fell Short

The vibe-coded version, generated mostly by an LLM, ended up being more complicated than necessary. It relied heavily on manual forwarding logic and custom handlers for each route, rather than leveraging YARP's built-in configuration and middleware pipeline. This resulted in:

- __More boilerplate:__ The code was longer and harder to follow, with repeated patterns and less abstraction.
- __Less idiomatic use of YARP:__ Instead of using YARP's configuration-driven routing and middleware, the LLM-generated code handled routing and forwarding manually, missing out on YARP's strengths.
- __Harder maintenance:__ The manual approach made it more difficult to extend, debug, or reason about the proxy's behavior.

In contrast, the hand-coded version uses YARP's configuration and pipeline features, resulting in much cleaner and more maintainable code. By relying on YARP's built-in mechanisms, the hand-coded approach is not only shorter and easier to read, but also more flexible and robust. It's easier to add new routing logic, integrate with observability tools, or update the proxy's behavior as requirements change.

__Conclusion:__

Vibe-coding can get you a working solution quickly, but it often produces code that is less maintainable and less idiomatic. Taking the time to understand the framework and write code yourself leads to better results in the long run.
